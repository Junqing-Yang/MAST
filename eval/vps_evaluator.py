...
def evaluator(gt_pth_lst, pred_pth_lst, metrics):
    ...
    for idx in tqdm(range(len(gt_pth_lst))):
        gt_pth = gt_pth_lst[idx].replace('\\','/')
        pred_pth = pred_pth_lst[idx].replace('\\','/')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=['TestEasyDataset/Unseen', 'TestHardDataset/Unseen'])
    parser.add_argument(
        '--model_lst', type=str, help='candidate competitors',
        nargs='+', action='append',
        choices=['23DCNN','AMD','DCFNet','FSNet','MAST','PCSA','PNSNet','SANet','UACA','ACSNet','COSNet','DCRNet','LDNet','MATNet','PNS+','PraNet','SSTAN','MAST'],
        default=['MAST','res_MAST'],)
    parser.add_argument(
        '--txt_name', type=str, help='logging root',
        default='pvtMAST_eval.txt')
    parser.add_argument(
