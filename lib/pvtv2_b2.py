...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if not self.linear:
            ...
            x_ = self.sr(self.pool(x_)).reshape(
                B, C, -1).permute(0, 2, 1).contiguous()
            x_ = self.norm(x_)
            x_ = self.act(x_)
            kv = self.kv(x_).reshape(B, -1, 2, self.num_heads, C //
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # nn.Identity(): 直接返回输入，无实质改变，增加网络层数
...
class OverlapPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        x = self.proj(x)   # Linear层
        _, _, H, W = x.shape
        x = x.flatten(2).transpose(1, 2).contiguous()  # flatten(2)： H*W
        x = self.norm(x)  # 归一化层
        return x, H, W


class PyramidVisionTransformerV2(nn.Module):
    def __init__(self, patch_size=16, img_size=224, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
...
def pvt_v2_b2(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        model_state = torch.load('./weight/pvt_v2_b2.pth',weights_only=True)
